...
function Wait-Connections($wb, [int]$TimeoutSec=1400){
  ...
  do {
    ...
    $any = $false
    # Connections may be $null on workbooks that only contain Power Query
    # definitions.  Enumerating @($null) yields a single $null element, so
    # explicitly skip nulls before checking the Refreshing property.  Without
    # this guard the script can throw "You cannot call a method on a
    # null-valued expression" when it attempts to reference properties on a
    # $null connection.
    foreach($cn in @($wb.Connections)){
      try {
        if ($null -ne $cn -and $cn.Refreshing){
          $any = $true
          break
        }
      } catch {
        # Swallow errors – not all connection types expose a Refreshing property
      }
    }
    if (-not $any){
      $sw.Stop();
...
function old_Refresh-WorkbookSmart([object]$excel,[string]$Path,[int]$TimeoutSec=900,[switch]$FastMode){
  ...
  try {
    ...
    try {
      # Ensure asynchronous Power Query refreshes finish before moving on.
    ...
    if (-not $FastMode){
      ...
      foreach($ws in @($wb.Worksheets)){
        try {
          # In PowerShell 7 and above, COM properties such as ListObjects and
          # PivotTables cannot be invoked like methods (e.g. ListObjects()).
          # Attempting to do so causes a "Method invocation failed" or
          # "You cannot call a method on a null-valued expression" error.  Use
          # property accessors instead and verify that the returned collection is
          # not $null before reading its Count property.
          $loColl = $ws.ListObjects
          if ($null -ne $loColl -and $loColl.Count -gt 0){
            $hasTables = $true
          }
        } catch {}
        try {
          $ptColl = $ws.PivotTables
          if ($null -ne $ptColl -and $ptColl.Count -gt 0){
            $hasPivots = $true
          }
        } catch {}
      }
    ...
    # Refresh the Power Pivot model if present and wait a few seconds for it to
    # finish.  The model property itself may be null on workbooks that do not
    ...
    try {
      # Perform a full calculation of all formulas in the workbook.  This call
      # occasionally raises COM exceptions if Excel is still busy, so catch and
      # ignore those errors.
      $excel.CalculateFull()
    } catch {}
    # Only attempt to save the workbook when Excel is no longer busy.  Wrap
    # Save() in a try/catch so that a failure does not bubble up as a null-valued
    # expression error.
    try {
      # Make sure all async queries are complete one more time before saving.
...
function Refresh-WorkbookSmart([object]$excel,[string]$Path,[int]$TimeoutSec=900,[switch]$FastMode){
  ...
  $wb = $null
  try {
    # Open workbook for read/write
    $wb = $excel.Workbooks.Open($Path,$false,$false)

    # Temporarily enable automatic calculation – required for CalculateUntilAsyncQueriesDone()
    $oldCalcMode = $excel.Calculation
    try { $excel.Calculation = -4105 } catch {}

    # Disable background refresh for ODBC/OLEDB connections in full mode
    if (-not $FastMode) {
      foreach ($cn in @($wb.Connections)) {
        try {
    ...
    # Refresh all queries and wait for them to finish
    $wb.RefreshAll() | Out-Null
    try { $excel.CalculateUntilAsyncQueriesDone() } catch {}
    [void](Wait-Connections -wb $wb -TimeoutSec $TimeoutSec)

    # Refresh tables and pivots if present (and not in FastMode)
    if (-not $FastMode) {
      $hasTables = $false; $hasPivots = $false
      foreach ($ws in @($wb.Worksheets)) {
        try { if ($ws.ListObjects -ne $null -and $ws.ListObjects.Count -gt 0) { $hasTables = $true } } catch {}
        try { if ($ws.PivotTables  -ne $null -and $ws.PivotTables.Count  -gt 0) { $hasPivots = $true } } catch {}
      }
      if ($hasTables) {
        foreach ($ws in @($wb.Worksheets)) {
    ...
    # Refresh the data model if present
    try { if ($wb.Model) { $wb.Model.Refresh(); Start-Sleep -Seconds 5 } } catch {}

    # Force a full recalculation and save
    try { $excel.CalculateFull() } catch {}
    $wb.Save()
  }
  finally {
    # Restore the calculation mode and close workbook
    try { if ($oldCalcMode -ne $null) { $excel.Calculation = $oldCalcMode } } catch {}
    if ($wb -ne $null) { try { $wb.Close($true) } catch {} }
  }
}
